...
//         {/* Animated Background Gradient */}
//         <motion.div
//           className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${contact.bgGradient}`}
//           initial={{ scale: 0.8 }}
//           animate={isHovered ? { scale: 1 } : { scale: 0.8 }}
//         />
...
//           {/* Floating Icon Container */}
//           <motion.div
//             className={`mb-6 p-5 rounded-2xl text-white shadow-xl shadow-emerald-900/20 group-hover:shadow-2xl transition-all duration-500 bg-gradient-to-br ${contact.gradient}`}
//             animate={isHovered ? {
//               y: [-5, 5, -5],
//               rotate: [0, 5, -5, 0],
//               scale: 1.1
...
//             </p>
//             <motion.p
//               className="text-base md:text-lg lg:text-xl font-black text-[#044116] transition-all break-words hover:text-transparent hover:bg-clip-text"
//               style={{
//                 backgroundImage: isHovered ? "linear-gradient(to right, #044116, #055a1a)" : "none",
...
export default function CTASection({ showFestiveLine = false }: CTASectionProps) {
  ...
  const ContactCard = ({ contact, index }: { contact: typeof contacts[0], index: number }) => {
    ...
    // Simplified interactions for mobile (touch devices don't always fire mouse move cleanly)
    return (
      <motion.a
        ref={cardRef}
        variants={itemVariants}
        ...
        className="group relative p-6 md:p-8 rounded-3xl bg-white border border-slate-200 hover:border-emerald-300 shadow-lg hover:shadow-2xl hover:shadow-emerald-900/10 transition-all duration-500 flex flex-col items-center text-center"
      >
        <motion.div
          className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${contact.bgGradient}`}
        />
        ...
        <div className="relative z-10 flex flex-col items-center w-full">
          <motion.div
            className={`mb-4 md:mb-6 p-4 md:p-5 rounded-2xl text-white shadow-xl shadow-emerald-900/20 group-hover:shadow-2xl transition-all duration-500 bg-gradient-to-br ${contact.gradient}`}
            animate={isHovered ? {
              y: [-5, 5, -5],
              rotate: [0, 5, -5, 0],
              scale: 1.1
          ...
          >
            <contact.icon className="w-6 h-6 md:w-10 md:h-10" strokeWidth={1.5} />
          </motion.div>
          ...
          <div className="space-y-2 md:space-y-3 mb-4 md:mb-6 w-full">
            ...
            <p className="text-base md:text-xl font-black text-[#044116] break-all">
              {contact.value}
            </p>
          </div>
  ...
  return (
    ...
    <section
      ref={ref}
      className="relative py-16 md:py-32 bg-gradient-to-b from-white via-emerald-50/30 to-white overflow-hidden"
    >
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(#044116_0.5px,transparent_0.5px)] [background-size:20px_20px] opacity-[0.02]" />
      </div>
      ...
      >
        <div className="space-y-12 md:space-y-16">
          <motion.div variants={itemVariants} className="text-center space-y-4 md:space-y-6">
            <motion.div
              whileHover={{ scale: 1.05 }}
              className="inline-block px-4 py-2 rounded-full bg-gradient-to-r from-emerald-100 to-emerald-50 border border-emerald-200 text-[#044116] font-black text-xs md:text-sm tracking-widest uppercase shadow-sm"
            >
              Get in Touch
            </motion.div>
            <motion.h2
              className="text-3xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#044116] via-[#055a1a] to-[#044116] tracking-tight"
            >
              Ready to Order?
            </motion.h2>
