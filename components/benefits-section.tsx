...
export default function BenefitsSection() {
  ...
  return (
    // Reduced padding for mobile: py-16 instead of py-24
    <section ref={ref} className="relative py-16 md:py-32 bg-gradient-to-br from-[#044116] via-[#033d14] to-[#044116] overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        ...
        <motion.div
          ...
          className="absolute top-[-10%] left-[-10%] w-[300px] h-[300px] md:w-[500px] md:h-[500px] bg-emerald-400/20 rounded-full blur-3xl"
        />
        <motion.div
          animate={{
            scale: [1, 1.4, 1],
          ...
          transition={{ duration: 25, repeat: Infinity, ease: "easeInOut" }}
          // Reduced size on mobile
          className="absolute bottom-[-10%] right-[-10%] w-[350px] h-[350px] md:w-[600px] md:h-[600px] bg-emerald-900/40 rounded-full blur-3xl"
        />
        {/* Sparkles kept as is */}
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10">
        ...
        >
          {/* Header */}
          <motion.div variants={itemVariants} className="text-center space-y-4 md:space-y-6 max-w-3xl mx-auto">
            <motion.div
              whileHover={{ scale: 1.05 }}
              className="inline-block px-4 py-1.5 md:px-5 md:py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-emerald-100 font-black text-xs md:text-sm tracking-widest uppercase shadow-lg"
            >
              Why Choose Us
            </motion.div>
            <motion.h2
            ...
            <p className="text-emerald-100/90 text-base md:text-xl max-w-2xl mx-auto leading-relaxed">
              We go the extra mile to ensure every box delivers happiness, health, and premium quality.
            </p>
          </motion.div>

          {/* Grid Layout */}
          {/* Adjusted Grid for better Mobile View (1 col -> 2 col or just cleaner 1 col) */}
          <motion.div
            variants={containerVariants}
            ...
            {benefits.map((benefit, idx) => {
              ...
              return (
                <motion.div
                  ...
                  variants={itemVariants}
                  whileHover={{ y: -12, scale: 1.02 }}
                  // Reduced padding to p-5 for mobile
                  className="group relative p-5 md:p-6 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl md:rounded-3xl hover:bg-white/10 hover:border-emerald-400/50 transition-all duration-500"
                >
                  <div className="relative flex flex-row md:flex-col items-center md:items-start gap-4 text-left">
                    {/* Animated Icon Box */}
                    <motion.div
                      className="p-3 md:p-3.5 rounded-xl md:rounded-2xl bg-gradient-to-br from-emerald-800 to-emerald-900 text-emerald-300 group-hover:from-emerald-600 group-hover:to-emerald-700 group-hover:text-white shadow-lg shrink-0"
                    ...
                    {/* Text */}
                    <p className="text-white/90 font-bold text-base md:text-lg leading-snug group-hover:text-white transition-colors">
                      {benefit.text}
                    </p>
                  </div>
                </motion.div>
